...
{
  int i,j,k, ii, jj, kk;
  for(i = 0; i < ARRAY_SIZE; i+=(ARRAY_SIZE/n)) //array_size/B
  {
    for(j = 0; j < ARRAY_SIZE; j+=(ARRAY_SIZE/n))
    {
      for(k = 0; k < ARRAY_SIZE; k+=(ARRAY_SIZE/n))
      {        
          for(ii = i; ii < i+(ARRAY_SIZE/n); ii++) //B
            for(jj = j; jj < j+(ARRAY_SIZE/n); jj++) // B
            //double sum = c[ii][jj]
              for(kk = k; kk < k+(ARRAY_SIZE/n); kk++) // B
                c[ii][jj] += a[ii][kk]*b[kk][jj]; // sum += a[ii][kk] * b[kk][jj]
              //c[ii][jj] = sum;
      }
    }
  }  
